...
enum L {
    ...
    static var finished: String { isTurkish ? "Bitti" : "Finished" }
    ...
    static var todaysMatches: String { isTurkish ? "Bugünün Maçları" : "Today's Matches" }
    static var loadingMatches: String { isTurkish ? "Maçlar yükleniyor..." : "Loading matches..." }
    static var couldNotLoad: String { isTurkish ? "Maçlar yüklenemedi" : "Could not load matches" }
    static var tryAgain: String { isTurkish ? "Tekrar Dene" : "Try Again" }
    static var noMatchesToday: String { isTurkish ? "Bugün maç yok" : "No matches today" }
    static var checkBackLater: String { isTurkish ? "Daha sonra tekrar kontrol edin" : "Check back later" }

    // MARK: - Watch Matches (Community)
    static var watchMatches: String { isTurkish ? "Watch Maçları" : "Watch Matches" }
    static var noCommunityMatches: String { isTurkish ? "Henüz başka oyuncu yok" : "No other players yet" }
    static var communityMatchesWillAppear: String {
        isTurkish
            ? "TennisWatch kullanan diğer oyuncuların\nmaçları burada görünecek"
            : "Matches from other TennisWatch\nplayers will appear here"
    }
}
